...
const dealRowSchema = z.object({
  ...
  leadExternalId: z.string().optional(),
...
const dealsIngestionSchema = z.object({
  companyId: z.string().min(1),
  rows: z.array(dealRowSchema).min(1).max(5000),
});
...
function deriveDealFlags(row) {
  const flags = [...(row.diagnosticFlags ?? [])];
  if (!row.leadExternalId) flags.push('missing_lead_external_id');
...
export class DealsService {
  ...
  ingest(payload) {
    const parsed = dealsIngestionSchema.safeParse(payload);
    if (!parsed.success) {
      return {
        ok: false,
        statusCode: 400,
    ...
    const existsStmt = this.db.prepare(`
      SELECT id
      FROM deals
      WHERE company_id = ? AND deal_external_id = ?
      LIMIT 1
    `);

    const upsertStmt = this.db.prepare(`
      INSERT INTO deals (
        id, company_id, deal_external_id, lead_external_id, lead_link_key, created_date, status, source_type,
    ...
    try {
      for (const row of rows) {
        const existing = existsStmt.get(companyId, row.dealExternalId);
        const diagnosticFlags = deriveDealFlags(row);
        ...
        const dbRow = {
          ...
          lead_external_id: normalizeText(row.leadExternalId),
        ...
        result.rows.push({
          dealExternalId: row.dealExternalId,
          leadExternalId: dbRow.lead_external_id,
          leadLinkKey: dbRow.lead_link_key,
