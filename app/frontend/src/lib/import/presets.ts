...
export const IMPORT_PRESETS: ImportPreset[] = [
...

  {
    ...
    fileType: 'deals',
    fields: [
      { field: 'dealExternalId', label: 'ID сделки', required: true, aliases: ['deal_id', 'id', 'id сделки', 'номер сделки', 'ид', 'код'] },
      { field: 'leadExternalId', label: 'ID лида', required: false, aliases: ['lead_id', 'lead', 'id лида', 'лид'] },
      { field: 'customerExternalId', label: 'ID клиента', required: false, aliases: ['client_id', 'customer_id', 'customer', 'клиент', 'id клиента', 'контрагент'] },
      { field: 'managerExternalId', label: 'Менеджер', required: false, aliases: ['manager_id', 'manager', 'менеджер', 'ответственный', 'sales_rep'] },
      { field: 'status', label: 'Статус', required: true, aliases: ['status', 'state', 'stage', 'статус', 'стадия', 'этап'] },
      ...
      { field: 'lostReason', label: 'Причина потери', required: false, aliases: ['lost_reason', 'loss_reason', 'причина', 'причина потери', 'причина отказа'] },
...

  {
    ...
    fileType: 'leads',
    fields: [
      { field: 'leadExternalId', label: 'ID лида', required: true, aliases: ['lead_id', 'id', 'id лида', 'номер', 'ид'] },
...

  {
    ...
    fields: [
      ...
      { field: 'customerExternalId', label: 'Клиент', required: false, aliases: ['client', 'customer', 'customer_id', 'клиент', 'контрагент', 'покупатель'] },
      { field: 'dealExternalId', label: 'Сделка', required: false, aliases: ['deal', 'deal_id', 'сделка', 'id сделки'] },
      { field: 'amount', label: 'Сумма', required: true, aliases: ['amount', 'total', 'sum', 'сумма', 'итого', 'стоимость'] },
      { field: 'invoiceDate', label: 'Дата счёта', required: false, aliases: ['date', 'invoice_date', 'issued', 'дата', 'дата счета', 'дата выставления'] },
      { field: 'dueDate', label: 'Срок оплаты', required: false, aliases: ['due_date', 'due', 'payment_due', 'срок', 'срок оплаты', 'дата оплаты'] },
      { field: 'status', label: 'Статус', required: false, aliases: ['status', 'state', 'payment_status', 'статус', 'оплачен'] },
    ],
...

  {
    ...
    fields: [
      ...
      { field: 'invoiceExternalId', label: 'Номер счёта', required: false, aliases: ['invoice_id', 'invoice', 'счет', 'номер счета'] },
      { field: 'amount', label: 'Сумма', required: true, aliases: ['amount', 'sum', 'total', 'сумма', 'поступление'] },
