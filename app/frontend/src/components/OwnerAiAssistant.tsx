    ...
    <Card className="chrona-surface border-primary/20">
      <CardHeader className="pb-2">
        ...
        <CardDescription className="text-sm">
          Ответ опирается на метрики и вывод периода, которые уже в облаке. Ключ OpenAI хранится только на сервере.
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-3">
        ...
        <Textarea
          placeholder="Например: почему падает конверсия и что проверить в продажах?"
          value={question}
        ...
        <div className="flex items-center justify-between gap-2">
          <p className="text-[10px] text-muted-foreground">Ctrl+Enter — отправить</p>
          <Button
            type="button"
            size="sm"
            disabled={busy || !context || !apiReady || !question.trim()}
            onClick={() => void submit()}
          >
